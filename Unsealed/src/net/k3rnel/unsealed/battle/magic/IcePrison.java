...
public class IcePrison extends MagicEntity {

    public IcePrison(TextureAtlas atlas, BattleEntity entity) {
        super(4.5f,0,entity);

        AtlasRegion atlasRegion = atlas.findRegion("battle/entities/icetacle");
        ...
        frames[1] = spriteSheet[3][0];
        frames[2] = spriteSheet[3][0];
        frames[3] = spriteSheet[3][1];
        frames[4] = spriteSheet[3][2];
        frames[5] = spriteSheet[3][3];
        ...
        if(this.getState()==BattleEntity.stateAttacking){
            if(currentAnimation.isAnimationFinished(stateTime)){
                ...
                if(BattleGrid.checkGrid(this.getGridXInt(),this.getGridYInt())!=null){
                    BattleEntity enemy = BattleGrid.checkGrid(this.getGridXInt(),this.getGridYInt());
                    if(enemy instanceof BattleEnemy){
                        enemy.setStatus(BattleEntity.statusFrozen);
                    }
                }
            }
        }
